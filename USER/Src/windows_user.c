//
// Created by 86187 on 2022/10/24.
//
/*
...
dspErrorStatus bartLettHannWin(uint16_t N, double w[]) {
    ...
    if ((N % 2) == 1) {
        for (n = 0; n < N; n++) {
            w[n] = 0.62 - 0.48 * fabs(((double) n / (N - 1)) - 0.5) +
                   0.38 * cos(2 * PI * (((double) n / (N - 1)) - 0.5));
        }
        for (n = 0; n < (N - 1) / 2; n++) {
            w[n] = w[N - 1 - n];
        }
...
double Besseli(double x, int n) {
    double Besseli_x = 1.0;

    for (int i = 1; i <= n; i++) {
        Besseli_x += pow((pow(x / 2, i) / FactorSeries(i)), 2);
    }
    return Besseli_x;
}
...
dspErrorStatus kaiserWin(uint16_t N, double beta, double w[]) {
    ...
    for (n = 0; n < N; n++) {
        theta = beta * sqrt(1 - pow(((2 * (double) n / (N - 1)) - 1), 2));
        *(w + n) = (double) Besseli(theta, BESSELI_K_LENGTH) / Besseli(beta, BESSELI_K_LENGTH);
    }
